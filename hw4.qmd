...
```{python}
crash_df[crash_df['zip_code'].isna() & crash_df['borough'].isna() & ~crash_df['latitude'].isna() & ~crash_df['longitude'].isna()]
```
...
```{python}
def fill_zip_and_borough(row):
  if (type(row['zip_code']) is float):
    row['zip_code'] = '10'
    return row
  else:
    return row
...
```{python}
df = crash_df.apply(fill_zip_and_borough, axis=1)
```
...
```{python}
crash_df['new_zip_code'] = [get_zip_code(x, y) for x, y in crash_df[['latitude', 'longitude']].iterrows()]
```
